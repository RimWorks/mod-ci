...
export async function shippedPackages(solution) {
  if (!solution) throw new Error('shippedPackages needs a solution path');
...
export async function writeStamp({ solution, modPath = process.cwd() } = {}) {
  ...
  // the old per-repo copies took no solution and read a hardcoded default, which is what let
  // the four of them drift apart
  const packages = solution ? await shippedPackages(solution) : [];
